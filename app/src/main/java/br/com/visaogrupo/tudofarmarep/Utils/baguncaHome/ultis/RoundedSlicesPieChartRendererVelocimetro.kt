...
class RoundedSlicesPieChartRendererVelocimetro (
    chart: PieChart?,
    animator: ChartAnimator?,
    viewPortHandler: ViewPortHandler?,
    val  listaCorFundo: List<Int>,
    val  listaCorTexto: List<Int>
            ...
            for (j in 0 until dataSet.entryCount) {
                ...
                if (dataSet.valueLineColor != ColorTemplate.COLOR_NONE) {
                    ...
                    val textPaint = Paint().apply {
                        color = listaCorTexto.get(j)
                        textSize = 30f
                        textAlign = Paint.Align.CENTER
                    }
                    val text =  dataSet.getEntryForIndex(j).y.toString() + "%"
                    val textBounds = Rect()
